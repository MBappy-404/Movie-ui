...
const OverviewStats = () => {
  ...
  const [displayMode, setDisplayMode] = useState<"earnings" | "count">(
    "earnings"
  );
      ...
      return (
        <div className="bg-[#00031b] p-4 rounded-lg border border-[#1a2d6d]">
          <p className="text-purple-400 font-semibold mb-2">
            {["7days", "30days", "90days"].includes(timeFilter)
              ? "Period"
              : "Month"}
            : {label}
          </p>
          <div className="space-y-1">
            ...
            <p className="text-sm text-purple-200">
              Earnings:{" "}
              <span className="ml-2">
                ${payload[0].payload.earnings.toLocaleString()}
              </span>
            </p>
          </div>
        </div>
    ...
    <div className="p-0 md:p-6 space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          value={data?.data?.totalPayment || "0"}
          trend="18.4%"
          icon="💳"
        ...
        <StatCard
          ...
          value={`$${data?.data?.totalEaring?._sum?.amount.toFixed(2) || "0"}`}
          trend="22.3%"
          icon="💰"
